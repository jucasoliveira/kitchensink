...
@Document("customers")
record CustomerDocument(@Id String userId, AccountDocument account) {
    ...
    static CustomerDocument from(Customer c) {
        ...
        return new CustomerDocument(c.userId(), new AccountDocument(c.account().status(), new ContactInfoDocument(
                ci.givenName(), ci.familyName(), ci.telephone(), ci.email(),
                new AddressDocument(a.streetName1(), a.streetName2(), a.city(), a.state(), a.zipCode(), a.country()))));
    }

    Customer toDomain() {
        ContactInfoDocument ci = this.account.contactInfo();
        AddressDocument a = ci.address();
        return new Customer(this.userId,
                new Account(this.account.status(), new ContactInfo(
                        ci.givenName(), ci.familyName(), ci.telephone(), ci.email(),
                        new Address(a.streetName1(), a.streetName2(), a.city(), a.state(), a.zipCode(), a.country()))));
    }
}
