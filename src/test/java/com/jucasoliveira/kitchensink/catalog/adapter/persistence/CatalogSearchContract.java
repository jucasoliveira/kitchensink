...
/**
 * Issue 3.2 and 3.3 — {@code SEARCH_ITEMS}, the seventh statement, and the only one that cannot
 * lose its {@code Locale} parameter. Run against both adapters, from one set of assertions.
 *
 ...
 *
 * <h2>The same behaviour, reached two ways</h2>
 *
 * <p><b>Mongo.</b> Not {@code $text}. A text index is word-stemmed, single-per-collection and
 * cannot express {@code like '%dog%'} — under it, "dog" would not find the DOGS category — and it
 * has no useful analyzer for the {@code ja_JP} and {@code zh_CN} rows this catalog is half made
 * of. The faithful shape is a case-insensitive regex over the same three fields, reached by one
 ...
 *
 * <p><b>JPA.</b> The statement comes back almost verbatim, keyword fragment and all — this is the
 * one place in the slice where the relational adapter is the simpler of the two, because it is
 * restoring SQL rather than replacing it. {@code key(idet) = :locale and key(pdet) = :locale} is
 * the legacy's {@code b.locale = ? and b.locale = c.locale}, so
 * {@link #search_is_scoped_to_one_locale} holds without a line of adapter logic.
 *
 * <p>One deliberate divergence, asserted at the bottom and shared by both adapters: the keyword is
 * neutralised — {@code Pattern.quote} under Mongo, {@code escape} under JPA — so a {@code %} typed
 ...
 */
@Tag("parity")
public abstract class CatalogSearchContract {
	...
	protected void a_keyword_matches_the_product_name() {
	...
	protected void matching_is_case_insensitive() {
	...
	protected void a_keyword_matches_a_substring_not_a_word() {
	...
	protected void a_keyword_matches_the_category_id() {
	...
	protected void a_keyword_matches_the_item_description() {
	...
	protected void keywords_are_ored_never_anded() {
	...
	protected void a_document_matching_twice_is_returned_once() {
		...
		// or $unwound carelessly would not — nor would a JPQL join that forgot its distinct.
	...
	protected void search_is_scoped_to_one_locale() {
		...
		// that reason, and both adapters have to drop it too — matching the product's ja_JP name
	...
	protected void a_keyword_does_not_leak_across_locales() {
	...
	protected void search_results_carry_no_promised_order() {
		// Stated as a test so that 3.4 sorts explicitly rather than inheriting whatever the store
		// happens to return, the way the JSPs inherited whatever Cloudscape happened to return.
		// It is also the assertion that lets one suite run against two stores that genuinely do
	...
	@Test
	@DisplayName("DIVERGENCE — '%' and '_' were LIKE wildcards in 2003 and are literals here, so they match nothing")
	protected void a_wildcard_character_is_a_literal_now() {
		// Under the legacy statement "%" + "%" + "%" made every non-null row match. Quoting the
		// keyword (Mongo) or escaping it (JPA) closes that, and closes the injection of ( ) [ ] . *
		// that a raw regex would open. '_' matched any single character and nobody noticed; it is
		// asserted here because it is the one wildcard the JPA adapter has to escape and the Mongo
		// adapter gets for free.
		assertThat(this.catalog.searchItems(Set.of("%"), EN)).isEmpty();
		assertThat(this.catalog.searchItems(Set.of(".*"), EN)).isEmpty();
		assertThat(this.catalog.searchItems(Set.of("_"), EN)).isEmpty();
	}
	...
	protected void no_keywords_matches_nothing() {
		assertThat(this.catalog.searchItems(Set.of(), EN)).isEmpty();
	}
