...
/**
 * Issue 3.2 — the six read statements of {@code CatalogDAOSQL.xml}, against the Mongo adapter,
 * through the port.
 *
 ...
 * {@link MongoCatalogSearchTest}.
 *
 * <p>The test speaks to {@link CatalogRepository} and never to the adapter, and it boots the real
 * application context rather than a {@code @DataMongoTest} slice — same reasoning as
 * {@code CustomerMongoRoundTripTest}: what is being proven is that the running application
 * resolves the port to the Mongo adapter under the default profile.
 *
 ...
 * {@code kitchensink.seed.catalog=true} and the {@code @Import} match
 * {@link CatalogSeedLoadTest} exactly, so both classes share one context and one container.
 *
 ...
 * read returns them all and choosing between them is 3.4's job.</li>
 ...
 * {@code zh_CN} would not match Cloudscape's anyway. The sort moves to the service, with a
 * {@code Collator} for the request locale, which is why the assertions below are
 * order-insensitive.</li>
 * </ol>
 */
@Tag("parity")
@SpringBootTest(properties = "kitchensink.seed.catalog=true")
@Import(TestcontainersConfiguration.class)
class MongoCatalogRepositoryTest {
	...
	void the_port_resolves_to_the_mongo_adapter() {
		...
		// the mongo package", not "the adapter is called X". Unwrapped in case @Repository's
		// exception-translation proxy sits in front of the bean.
	...
	@Test
	@DisplayName("category_details.descn — the legacy seed carries no category description, and the adapter invents none")
	void a_category_description_is_null_because_the_seed_has_none() {
		...
		// rather than look like a mapping gap when 3.5 renders the sidebar.
		Category category = this.catalog.findCategory("FISH").orElseThrow();
	...
	void get_item_is_one_find_by_id() {
		...
		// call returns every locale the seed holds for EST-1.
	...
	void the_product_page_projection_takes_two_reads() {
	...
	void a_miss_is_an_empty_optional() {
	...
	void an_empty_result_is_an_empty_list() {
	...
	void every_seeded_document_is_reachable() {
