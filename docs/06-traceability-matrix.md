...
the same port (3.3) — so the catalog read path now answers under both persistence profiles. Still
planned: T1's remaining widening (credit card, profile, full URL-protection parity) and the catalog
screens (3.5).
